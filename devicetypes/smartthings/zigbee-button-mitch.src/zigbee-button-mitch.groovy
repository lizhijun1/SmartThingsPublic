...
metadata {
    definition (name: "ZigBee Button Mitch", namespace: "smartthings", author: "Mitch Pond", runLocally: true, minHubCoreVersion: "000.022.0002", executeCommandsLocally: false) {
        capability "Actuator"
        capability "Battery"
        capability "Button"
        ...
        fingerprint inClusters: "0000, 0001, 0003, 0007, 0020, 0402, 0B05", outClusters: "0003, 0006, 0019", manufacturer: "CentraLite", model: "3460-L2", deviceJoinName: "Iris Smart Button"
...
private Map parseNonIasButtonMessage(Map descMap){
    ...
    else if ((device.getDataValue("model") == "3450-L2") && (descMap.clusterInt == 0x0006)) {
        if (descMap.commandInt == 1) {
            getButtonResult("press")
        }
        else if (descMap.commandInt == 0) {
...
def configure() {
    ...
    def cmds = []
    if (device.getDataValue("model") == "3450-L2") {
        cmds << [
...
def initialize() {
    ...
    if ((device.getDataValue("manufacturer") == "OSRAM") && (device.getDataValue("model") == "LIGHTIFY Dimming Switch")) {
        ...
        else if (device.getDataValue("model") == "3450-L2") {
